(() => {
  ...
  const fillGeneralDetails = async (root, profile) => {
    ...
    const entries = [
      ...
      [["state", "province"], details.state, true],
      [["country", "nation"], details.country, true],
      [["pincode", "pin code", "postal code", "zip code"], details.pincode, false]
    ].filter(([, value]) => value !== undefined && value !== null && value !== "");
    let filled = 0;
    for (const [keywords, value, custom] of entries) if (await fillWithRetry(root, keywords, value, used, custom)) filled++;
  ...
  const fillPilgrim = async (root, pilgrim) => {
    ...
    const entries = [
      ...
      [["gender", "sex"], pilgrim.gender, true],
      [["photo id proof", "id proof", "proof type", "document type", "identity proof"], pilgrim.idType, true],
      [["photo id number", "id number", "proof id number", "document number", "identity number"], pilgrim.idNumber, false]
    ].filter(([, value]) => value !== undefined && value !== null && value !== "");
    let filled = 0;
    for (const [keywords, value, custom] of entries) if (await fillWithRetry(root, keywords, value, used, custom)) filled++;
    return { filled, expected: entries.length };
    ...
    (async () => {
      try {
        ...
        if (message.action === "fillAndContinue" && complete) {
          ...
          sendResponse({ message: continued ? `Filled and continued (${filled}/${expected} fields).` : `Filled all ${filled} fields, but the next-page button was not found.` });
        } else {
          sendResponse({ message: complete ? `Filled all ${filled} field(s). Review details before continuing.` : `Filled ${filled}/${expected} field(s). Some fields may still be loading; review them before continuing.` });
        }
      } catch (error) {
