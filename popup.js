...
const defaultData = {
  profiles: [{ id: crypto.randomUUID(), name: "Family passengers", passengers: [emptyPassenger()] }],
  selected: null,
  preferences: { autoUpgrade: false, confirmedBerths: false }
};
let data;
...
async function load() {
  ...
  data.profiles = (data.profiles?.length ? data.profiles : defaultData.profiles).map(migrateProfile);
